...
test("buildArticleListing paginates all articles by 6 per page", () => {
  ...
  assert.equal(listing.pagination.totalItems, 7);
  assert.deepEqual(
...
test("buildArticleListing exposes category facets with counts", () => {
  const listing = buildArticleListing(sampleArticles, {});

  assert.deepEqual(listing.facets.slice(0, 3), [
    { label: "全部", value: "all", count: 7 },
    { label: "法律翻译", value: "法律翻译", count: 2 },
    { label: "法律合规", value: "法律合规", count: 2 },
  ]);
  assert.equal(listing.facets.find((facet) => facet.value === "专业翻译")?.count, 1);
});
