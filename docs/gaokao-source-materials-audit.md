...
其中 `范文.txt` 的经验已经以抽象规则形式进入 `gaokao_writing_checklist.md` 的「范文生成质量控制」小节：

- 约束 `safe_version`：保留学生原意、补足题目要点、修正基础错误，保持考场稳定和可模仿。
- 约束 `advanced_version`：提高词汇精确度、句式层次和衔接质量，但不得跑题、堆砌或大学议论文化。
- 明确两版范文必须拉开层级：稳妥版解决“写对、写完整”，进阶版解决“更成熟、更紧凑”。
- 明确范文长度需贴合题型：传统单篇约 100-120 词，新高考应用文约 80 词，读后续写约 150 词。
- 明确不得把参考范文中的主题、观点或句子直接搬进用户报告。
...
6. 每次更新 `范文.txt` 提炼规则或 `gaokao_writing_checklist.md` 的范文生成质量控制部分，也必须跑质量闸门批测。
