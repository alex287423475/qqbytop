...
- 生产诊断必须加载 `backend/prompts/gaokao_scoring_rubric.md`，使用 25 分制五档锚点：21-25 分、16-20 分、11-15 分、6-10 分、1-5 分。`score.estimated` 是整体分档判断，不得由维度分简单相加。
- 首版默认传统单篇作文目标区间为 100-120 词；少于 80 词或多于 120 词应标记字数风险并提示约 2 分扣分风险，低于 70 词应显著降低内容完成度判断。新高考/上海卷后续若支持应用文约 80 词、读后续写约 150 词，必须通过 `task_type` 配置进入诊断。
- 生产诊断必须加载 `backend/prompts/gaokao_error_taxonomy.md`，将高考作文常见错误沉淀为内部错误分类，覆盖审题偏离、要点缺漏、连接词误用、中式英语、词性错误、搭配错误、时态/语态、主谓一致、句式单一和伪高级表达等。`highlight_spans.category` 应优先使用该 taxonomy 的类别或中文等价表达，不得泛泛写成“语法问题”。
