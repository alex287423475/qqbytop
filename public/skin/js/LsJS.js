...
var LR_c138='';var LR_c139='';var LR_c140='';var LR_pm016='';	if('1'=='1'){LR_showfloat=0;}
	var _lr_helpsrc_on=unescape("http://pht.zoosnet.net/lr/images/online_cn3.gif");
	...
	var _lr_helpsrc_of=unescape("http%3a%2f%2fpht.zoosnet.net%2flr%2fimages%2foffline_cn3.gif");
...
var data='jid='+LR_js_pm+'&id='+LR_siteid+'&sid='+LR_sid+'&cid='+LR_cid+'&lng='+LR_lng;
	...
	data+='&r='+lr_refer5237;
	data+='&e='+escape(LR_explain)+'&NOAC=FGAB&NPAC=GHAB&OPAC=HHAB&IKEN='+ lr_para3 + '&s=' + LR_sSize;
...
function LR_minisrc() {
    ...
    if (LR_robot != '') {
        ...
        return LR_robot + '?siteid=' + LR_websiteid + '&sid=' + LR_sid + '&cid=' + LR_cid + '&lng=' + LR_lng + '&keyword=' + LR_skey + '&lastcusname=&webtype=pcmini&p=' + escape(location.href) + '&r=' + referInfoTemp + "&p1=" + (typeof (LR_purl) == "string" ? escape(LR_purl) : "") + "&e=" + (typeof (LR_explain) != 'undefined' && LR_explain != '' ? escape(LR_explain) : "") + "&msg=" + (typeof (LR_msg) != 'undefined' && LR_msg != '' ? escape(LR_msg) : "");
...
var _check_LR_mini_js_pctype = function () {
    if (typeof (LR_mini_js_pctype) == "number") {
        return LR_mini_js_pctype;
    }
    return ("0");
}

var _lr_new_mini = false;
...
function minibarhtmlF(islrminimin) {
    ...
    if (_check_LR_mini_js_pctype() == "4") {

        var _LR_oldcss = document.querySelectorAll("link");

        for (var i = 0; i < _LR_oldcss.length; i++) {
            if (_LR_oldcss[i].hasAttribute("href")) {

                if (_LR_oldcss[i].href.toLowerCase().indexOf("/js/js5.css") > -1)
                    _LR_oldcss[i].remove();
            }
        }

        var eLR_newCss = document.getElementById("lr_mini_css");

        if (!eLR_newCss) {

            var eLR_miniCss = document.createElement("link");
            eLR_miniCss.type = "text/css";
            eLR_miniCss.rel = "stylesheet";
            eLR_miniCss.href = LR_imgurl + 'js/JS6.css';
            eLR_miniCss.id = "lr_mini_css";

            if (document.head)
                document.head.appendChild(eLR_miniCss);
        }

        return '<div onclick="lrminiMax()" id="LRMINIBar" class="' + (LR_pm011 ? 'lr-mini-icon-left' : '') + '" style="display:' + (!islrminimin ? 'none' : 'flex') + ';' + (LR_pm011 ? 'left' : 'right') + ': 50px;bottom: ' + LR_pm012 + 'px;" ><div class="lr-chat-text"  style="display:none;"><div id="LR-BUBBLE-MSG" style="display:none;"></div></div><div class="LR-BTN"><div id="LR-CIRCLE"  style="display:none;"></div><img src="' + LR_imgurl + 'js/duihua.svg" src="duihua.svg" width="36"></div></div>';
    }
    else {
        ...
        window.addEventListener('message', function (e) {
            ...
            if (lr_cmd.a == 'newmsg') {
                ...
                if (_check_LR_mini_js_pctype() == "4") {
                    ...
                    if (LR_GetObj('LRMINIBar').style.display == 'flex') {
                        lr_newcount++;
                        var _obj = LR_GetObj('LR-CIRCLE'); _obj.innerHTML = lr_newcount; _obj.style.display = 'block';
                        LR_GetObj('LR-BUBBLE-MSG').parentNode.style.display = LR_GetObj('LR-BUBBLE-MSG').style.display = (!lr_cmd.d || !lr_cmd.c) ? 'none' : 'flex';

                        var lr_chat_text = LR_GetObj('LR-BUBBLE-MSG').parentNode;

                        LR_GetObj('LR-BUBBLE-MSG').remove();
                        var eLR_Bubble_msg = document.createElement("div");
                        eLR_Bubble_msg.id = "LR-BUBBLE-MSG";
                        eLR_Bubble_msg.style.display = "flex";
                        lr_chat_text.appendChild(eLR_Bubble_msg);

                        LR_GetObj('LR-BUBBLE-MSG').innerHTML = NewMsgNoHtml( lr_cmd.b);
                        return;
                    }
                }
                else {
            ...
            if (lr_cmd.a == "newmini") {
                if (LiveReceptionCode_isonline) {
                    LR_GetObj('LRMINIWIN').style.width = "510px";
                    LR_GetObj('LRMINIWIN').style.height = "626px";

                    LR_GetObj('LRMINIWIN0').style = "z-index: 2147483647; position: absolute; height: 55px; overflow: hidden; display: block;top:0;left:0;width: 510px;";

                    LR_GetObj('LRMINIWIN0').innerHTML = '<div style="width: 340px;height: 100%;float: left;cursor: move;margin-left:56px;" id="LR_Drag_Box"></div><span style="float:left;clear:none; width:3px;height:20px; overflow:hidden; margin:0 5px;"></span><span style="' + (LR_pm007 ? '' : 'display:none;') + 'CURSOR:pointer;float:right;clear:none; width:30px;height:32px; margin:6px 2px 0 0; overflow:hidden;" onclick="lr_closemini(1);"><img style="cursor:pointer;width:12px;color:#000;" src="' + LR_imgurl + 'js/guanbi.svg" border="0"></span><span style="CURSOR:pointer;float:right;overflow:hidden;width:30px;height:32px;clear:none;display:block;margin:6px 2px 0 0;" onclick="openZoosUrl(\'bchatwin\');lrminiMin();"><img src="' + LR_imgurl + 'js/zuida.svg" style="cursor:pointer;width:12px;color:#000;" border="0"></span><span style="CURSOR:pointer;float:right;overflow:hidden;width:30px;height:32px;clear:none;display:block;margin:6px 2px 0 0;" onclick="lrminiMin();"><img src="' + LR_imgurl + 'js/zuixiao.svg" style="cursor:pointer;width:12px;color:#000;" border="0"></span>';

                    LR_GetObj('LR_miniframe').style.height = "626px";

                    var _eDomDragLeft = document.createElement("div");
                    _eDomDragLeft.id = "LR_Drag_Box_Left";
                    _eDomDragLeft.style = "z-index: 2147483647;position: absolute;height: 380px;overflow: hidden;display: block;left:0;width: 56px;bottom: 0px;cursor: move;";

                    LR_GetObj('LRMINIWIN').appendChild(_eDomDragLeft);
                    _lr_new_mini = true;
                    BindLR_Drag();
                }
            }
...
function LR_showminiDiv(islrminimin, data) {
    if ((LR_chatkind == -1 && (!LiveReceptionCode_isonline && LR_robot.length == 0)) && !clickopenmini) {
...
function LR_showminiDiv_of() {
    ...
    if (!document.body) {
        setTimeout('LR_showminiDiv_of();', 500);
        return;
    }
...
function lrminiMin(nop) {
    ...
    if (_check_LR_mini_js_pctype() == "4") {
        LR_GetObj('LRMINIBar').style.display = 'flex';
    }
    else {
...
function lrminiMax() {
    ...
    if (_lr_new_mini) {
        LR_GetObj('LRMINIWIN').style.width = LiveReceptionCode_isonline ? '510px' : '420px';
    }
    else {
    ...
    if (LR_GetObj('LR-CIRCLE')) {
        ...
        if (_check_LR_mini_js_pctype() == "4") {
            LR_GetObj('LR-BUBBLE-MSG').parentNode.style.display = LR_GetObj('LR-BUBBLE-MSG').style.display = 'none';
        }
        else {
            LR_GetObj('LR-BUBBLE').style.display = 'none';
        }
...
function LR_showminiDiv2() {
    if (typeof (LR_showminiDiv_no) != 'undefined' || !LR_pm001 || (!LiveReceptionCode_isonline && LR_robot.length == 0) || !LR_pm002 || LR_sidexists == 2) return;
    if (LR_cid == null) {
...
var bIsBindLR_Drag = false;
var BindLR_Drag = function () {
    if (bIsBindLR_Drag)
        return;

    bIsBindLR_Drag = true;

    const draggable = document.getElementById('LR_Drag_Box');

    const draggableLeft = document.getElementById('LR_Drag_Box_Left');

    const dragBox = document.getElementById('LRfloater2');

    dragBox.style.display = "flex";
    dragBox.style.position = "fixed";


    let initLeft = getPosition(dragBox).left;
    let initTop = getPosition(dragBox).top;
    ...
    //    startPos.left = getPosition(dragBox).left;
    //    startPos.top = getPosition(dragBox).top;
    //    isDragging = true;
    //});
    ...
    BindDragFun(draggable);
    BindDragFun(draggableLeft);
    ...
    document.addEventListener('mousemove', function (e) {
        if (isDragging) {
            ...
            let _top = diffY + startPos.top;
            let _left = diffX + startPos.left;
            let maxTop = window.innerHeight - dragBox.offsetHeight;
            let maxLeft = window.innerWidth - dragBox.offsetWidth;

            console.log(window.innerWidth, dragBox.offsetWidth);
            //限制 _top 和 _left
            //限制_top 最小值不能小于 0
            ...
            if (dragBox.style.cssText.indexOf("bottom")) {
                dragBox.style.removeProperty("bottom");
            }
            if (dragBox.style.cssText.indexOf("right")) {
                dragBox.style.removeProperty("right");
            }


            dragBox.style.top = _top + 'px';
            dragBox.style.left = _left + 'px';
