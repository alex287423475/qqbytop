...
async function loadFixedStandardSample() {
  try {
    const response = await fetch("/api/standard-sample", { cache: "no-store" });
...
async function loadFixedProfessionalSample() {
  try {
    const response = await fetch("/api/professional-sample", { cache: "no-store" });
    if (!response.ok) return false;
    const sample = await response.json();
...
async function initSampleAdminMode() {
  ...
  try {
    const response = await fetch("/api/sample-admin", {
...
async function retryReferenceImage(index, item, button) {
  try {
    ...
    const response = await fetch("/api/generate-single-reference", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        scenario: state.scenario,
...
async function saveFixedProfessionalSampleFromCurrentResult() {
  ...
  await saveFixedSample({
    ...
    endpoint: "/api/professional-sample",
    sample,
